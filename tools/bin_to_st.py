...
for k in list(vista_bin.keys()):  # merge LoRA weights (if exist) for inference
    if "adapter_down" in k:
        ...
        if pretrain_k not in vista_bin.keys():  # then we need to resort to EMA weights
            ema_pretrain_k = pretrain_k.replace(".", "").replace("modeldiffusion_model", "model_ema.diffusion_model").replace("_forward_module", "_forward_module.")
            vista_bin[pretrain_k] = vista_bin[ema_pretrain_k] + lora_weights
            del vista_bin[ema_pretrain_k]
        else:
...
save_file(vista_st, "ckpts/unifuture.safetensors")
