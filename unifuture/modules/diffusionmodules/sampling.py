...
from unifuture.modules.diffusionmodules.sampling_utils import to_d
from unifuture.util import append_dims, default, instantiate_from_config
...
class BaseDiffusionSampler:
    ...
    def denoise(self, x, denoiser, sigma, cond, cond_mask, uc):
        denoised, depth = denoiser(*self.guider.prepare_inputs(x, sigma, cond, cond_mask, uc))
        denoised = self.guider(denoised, sigma)
        if depth is not None:
            depth = self.guider(depth, sigma)
        return denoised, depth
...
class EulerEDMSampler(SingleStepDiffusionSampler):
    ...
    def sampler_step(self, sigma, next_sigma, denoiser, x, cond, cond_mask=None, uc=None, gamma=0.0):
        ...
        denoised, depth = self.denoise(x, denoiser, sigma_hat, cond, cond_mask, uc)
        d = to_d(x, sigma_hat, denoised)
        dt = append_dims(next_sigma - sigma_hat, x.ndim)

        euler_step = self.euler_step(x, d, dt)
        return euler_step, depth
